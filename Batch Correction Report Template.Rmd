...
This interactive R Markdown document provides tools and summaries to help students, researchers, and technicians identify batch effects in their metabolomic data sets, optimize appropriate batch correction algorithms, and export corrected matrices. These features are separated into the following three sections: 
  ...
  3. Lastly, an export function is available for the final batch corrected data set. Additionally, normalization transformers and feature filters are provided which can be set based on user criteria. The exported data is saved with a brief summary of the data transformations applied, along with any filters, to ensure traceability and repeatability of the final feature table. 
...
Each section provides users with an opportunity to select a data transformation, thus the transformation selected here will only be applied to section 1. Section 3 provides an more in depth analysis the transformation has on the distribution of each metabolite. This is provided so an appropriate transformation, which aligns with the requirements/assumptions of statistical analyses required by users, can be selected. 
...
The following plot summarizes the improvment the batch-correction had on technical variation (select "QC" under "Class Selection") and biological variation (select "Sample" under "Class Selection"). The x-axis "CV-Fold Change" is a ratio of the uncorrected CV to the corrected CV. Thus a value greater than one indicates a reduction in variation. The y-axis "Post-Correction CV" indcates the variation following correction. The option to include labels can be used to help identify outliers following correction.
...
create_table(data = raw_data, transform_input = input$transformer_section_2, corrected_data = FALSE)[[1]]
  
})

renderText({
  
  create_table(data = raw_data, transform_input = input$transformer_section_2, corrected_data = FALSE)[[2]]
...
2. Interclas correlation Coefficient 

Provide a file name for a csv of the final data set. 
